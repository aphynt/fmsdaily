...
@php
    $perEx = collect($data['kategori']['GroupedPerExHour'])
                ->groupBy('LOD_LOADERID');
@endphp

<div class="accordion" id="accordionPerExShift">

@foreach ($perEx as $ex => $rows)
...
@php
    ...
    $chartSiang = 'chart_siang_'.$loop->index;
    $chartMalam = 'chart_malam_'.$loop->index;
@endphp
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    /* ================= DATA SIANG ================= */
    const dataSiang_{{ $loop->index }} = @json($siang);
    const orderSiang = getUrutanJam('Siang');
    ...
    const labelsSiang_{{ $loop->index }} = orderSiang.map(h =>
        String(h).padStart(2,'0')
    );

    const prodSiang_{{ $loop->index }} = orderSiang.map(h =>
        mapSiang[h] ? Math.round(mapSiang[h].PRODUCTION) : 0
    );
    ...
    if (document.querySelector('#chart_siang_{{ $loop->index }}')) {
        new ApexCharts(
            document.querySelector('#chart_siang_{{ $loop->index }}'),
            {
                ...
                series: [{
                    name: 'Production',
                    data: prodSiang_{{ $loop->index }}
                }],
                xaxis: {
                    categories: labelsSiang_{{ $loop->index }},
                    title: { text: 'Jam' }
                },
                plotOptions: {
                    bar: {
                ...
                dataLabels: {
                    enabled: true,
                    offsetY: -10,                 // posisikan lebih rapi di tengah bar
                    rotate: -90,                 // ROTATE KE ATAS (vertical)
                    style: {
                    ...
                    formatter: function (val, opts) {
                        ...
                        const row = dataSiang_{{ $loop->index }}[i];

                        const prod = Math.round(row.PRODUCTION);
                        const plan = Math.round(row.PLAN_PRODUCTION);
    ...
    /* ================= DATA MALAM ================= */
    const dataMalam_{{ $loop->index }} = @json($malam);
    const orderMalam = getUrutanJam('Malam');
    ...
    const labelsMalam_{{ $loop->index }} = orderMalam.map(h =>
        String(h).padStart(2,'0')
    );

    const prodMalam_{{ $loop->index }} = orderMalam.map(h =>
        mapMalam[h] ? Math.round(mapMalam[h].PRODUCTION) : 0
    );
    ...
    if (document.querySelector('#chart_malam_{{ $loop->index }}')) {
        new ApexCharts(
            document.querySelector('#chart_malam_{{ $loop->index }}'),
            {
                ...
                series: [{
                    name: 'Production',
                    data: prodMalam_{{ $loop->index }}
                }],
                xaxis: {
                    categories: labelsMalam_{{ $loop->index }},
                    title: { text: 'Jam' }
                },
                plotOptions: {
                    bar: {
                ...
                dataLabels: {
                    ...
                    formatter: function (val, opts) {
                        const i = opts.dataPointIndex;
                        const row = dataMalam_{{ $loop->index }}[i];
